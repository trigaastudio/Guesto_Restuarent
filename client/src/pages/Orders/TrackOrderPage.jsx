...
const TrackOrderPage = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#FAF9F6] font-sans ${theme}`}>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-10 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            <button
              ...
              className="flex items-center gap-2 text-[10px] font-black text-[#D10000] uppercase tracking-[0.2em] mb-3 hover:gap-3 transition-all"
            >
              <ArrowLeft size={14} /> Back to My Orders
            </button>
            <h1 className="text-4xl font-black text-text-primary tracking-tighter">Track Order <span className="text-[#D10000]">#{order.orderNumber || order._id.slice(-8).toUpperCase()}</span></h1>
          </div>
          <div className="bg-white px-6 py-4 rounded-[1.5rem] border border-gray-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Estimated Arrival</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-[0_20px_60px_rgba(0,0,0,0.02)] relative overflow-hidden">
              {/* Status Stepper */}
                ...
                {statusSteps.map((step, index) => {
                  ...
                  return (
                    <div key={step.id} className="relative flex gap-8 group">
                      {/* Vertical Line */}
                      {!isLast && (
                        <div className={`absolute left-7 top-14 bottom-[-48px] w-1 rounded-full transition-colors duration-1000 ${index < activeStepIndex ? 'bg-[#D10000]' : 'bg-gray-100'}`}></div>
                      )}

                      {/* Icon Node */}
                      <div className={`relative z-10 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-700 shadow-lg ${isCompleted ? 'bg-[#D10000] text-white shadow-[#D10000]/20' : 'bg-gray-50 text-gray-300'}`}>
                        {isActive && (
                          <div className="absolute inset-0 bg-[#D10000] rounded-2xl animate-ping opacity-20"></div>
                        )}
                        {step.icon}
                      </div>
                      ...
                      <div className="flex-1 pt-1">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className={`text-lg font-black transition-colors duration-700 ${isCompleted ? 'text-text-primary' : 'text-gray-300'}`}>{step.label}</h3>
                            <p className={`text-xs font-bold transition-colors duration-700 ${isCompleted ? 'text-text-muted' : 'text-gray-200'}`}>{step.description}</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {activeStepIndex >= 2 && (
              <div className="bg-[#D10000] rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-[#D10000]/30 relative overflow-hidden">
                ...
                <div className="flex items-center gap-6 relative z-10">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black opacity-80 uppercase tracking-widest">Premium Partner</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-4 relative z-10 w-full md:w-auto">
                  ...
                  <a href="tel:+919876543210" className="flex-1 md:flex-none h-14 px-8 rounded-2xl bg-white text-[#D10000] font-black flex items-center justify-center gap-3 shadow-xl hover:scale-105 active:scale-95 transition-all text-sm uppercase tracking-widest">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-black text-[#D10000] uppercase tracking-widest mt-2">{order.address?.mobile}</p>
              </div>
            </div>

            {/* Order Items */}
            <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
              <h3 className="text-lg font-black text-text-primary tracking-tight mb-6 uppercase">Order Summary</h3>
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-sm font-black text-text-primary uppercase tracking-widest">Total Paid</span>
            ...
            <div className="bg-gray-50 rounded-[2.5rem] p-8 border border-gray-100 text-center">
              ...
              <button className="w-full py-4 rounded-2xl border-2 border-gray-200 text-xs font-black uppercase tracking-widest hover:bg-white transition-all active:scale-[0.98]">Contact Support</button>
            </div>
          </div>
        </div>
      </main>
